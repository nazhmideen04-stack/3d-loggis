...
def fetch_loggis_data(target_date_str=None):
    """Динамически извлекает список дат со страницы LoggIS и парсит значения."""
    dates_list = []
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Динамически считываем все доступные даты из селектора LoggIS
            try:
                date_combobox = page.get_by_role("combobox").nth(1)
                options = date_combobox.locator("option").all_inner_texts()
                dates_list = [opt.strip() for opt in options if opt.strip() and opt.strip() != "TABLE_ROW_DATE"]
            except Exception:
                pass

            # Выбор конкретной исторической даты из базы
...
# Сначала получаем список дат и актуальные данные
...
# Загружаем данные выбранного периода, если это не текущий
