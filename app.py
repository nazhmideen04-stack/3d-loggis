...
def _wait_data_loaded(page, timeout=45000):
    """Ждём, пока исчезнут «Chargement en cours» / «Récupération des données...»."""
    try:
        page.wait_for_function(
            """() => !Array.from(document.querySelectorAll('body *')).some(e =>
                   e.children.length === 0 && e.offsetParent !== null &&
                   /Récupération des données|Chargement en cours/i.test(e.textContent || ''))""",
            timeout=timeout)
    except Exception:
        pass
    page.wait_for_timeout(1000)
...
def _download_csv(page):
    """Как в записи: expect_download( expect_popup( click '🠋CSV' ) ), попап закрывается ПОСЛЕ скачивания."""
    ...
    popup = None
    try:
        with page.expect_download(timeout=90000) as d_info:
            try:
                with page.expect_popup(timeout=15000) as p_info:
                    csv_btn.click()
                popup = p_info.value
            except Exception:
                pass
        download = d_info.value
        path = download.path()          # ждёт окончания скачивания
...
def _fetch_live_cached(mode_type):
    ...
    with sync_playwright() as p:
        ...
        try:
            _open_loggis(page, mode_type, view_type="TABLE_ROW_DATE")

            for cat_key, cat_cfg in CATEGORIES.items():
                if live_db[cat_key]:
                    continue
                _select_category(page, cat_cfg)
                page.wait_for_timeout(2000)
                ...
                # 1) таблица на странице
                cat_key_ts, vals = None, {}
                try:
                    tbl = _pick_sensor_table(page.evaluate(_TABLE_JS))
                    if tbl:
                        dt, vals, _ = _last_row_values(tbl)
                        cat_key_ts = ts_key(dt) if dt else None
                except Exception as e:
                    print(f"Tablo okuma hatası ({cat_key}): {e}")

                # 2) если таблица не прочиталась — тот же период (MONTH_02) через CSV, последняя строка
                if not vals:
                    path = _download_csv(page)
                    if path:
                        parsed, _ = parse_loggis_csv(path)
                        cat_key_ts, vals = _latest_from_parsed(parsed)

                for s_name, v in vals.items():
                    c = classify_sensor(s_name)
                    if c:
                        live_db[c][s_name] = v
                if cat_key_ts and (latest_key is None or cat_key_ts > latest_key):
                    latest_key = cat_key_ts
        finally:
            browser.close()

    if not any(live_db.values()):
        raise RuntimeError("LoggIS'ten canlı veri alınamadı.")   # пустой результат не кэшируется
...
def _fetch_csv_cached(mode_type):
        ...
        try:
            ...
            for cat_key, cat_cfg in CATEGORIES.items():
                ...
                if not path:
                    continue
                parsed, _ = parse_loggis_csv(path)
                # раскладываем по категории по имени сенсора
    ...
    if not all_keys:
        raise RuntimeError("LoggIS'ten CSV arşiv verisi alınamadı.")   # пустой результат не кэшируется
