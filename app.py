...
# =========================================================================
# 1. ТЕКУЩИЕ ДАННЫЕ (Идеальный DOM Парсер с textContent и фильтром ALL)
# =========================================================================
...
def fetch_current_data():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ТОЧНЫЙ АЛГОРИТМ ИЗ ТВОЕГО ФАЙЛА
            ...
            # ИСПРАВЛЕНИЕ: Выбираем ALL вместо MONTH_02, чтобы получить САМЫЕ СВЕЖИЕ данные!
            ...
            for cat_key, cat_cfg in CATEGORIES.items():
                ...
                for _ in range(15):
                    try:
                        # ИСПРАВЛЕНИЕ: Используем textContent вместо innerText, чтобы читать 100% колонок!
...
if data_mode == "🔴 Canlı (Güncel) Veriler":
    with st.spinner("En yeni canlı veriler alınıyor (Hızlı Tarama)..."):
...
<body>
    ...
    <script>
        ...
        gltfLoader.parse(bytes.buffer, '', function(gltf) {
            ...
            // ИДЕАЛЬНОЕ ЦЕНТРИРОВАНИЕ КАМЕРЫ (С ПРИБЛИЖЕНИЕМ х2.5)
            if (payload.selectedSensor && payload.selectedSensor !== "Seçiniz..." && selectedMeshRef) {
                ...
                if (!tunnelBox.isEmpty()) {
                    ...
                    // Увеличено приближение камеры: множитель 0.35 (было 0.45)
                    const fov = camera.fov * (Math.PI / 180);
                    let cameraZ = Math.abs(maxDim / Math.sin(fov / 2)) * 0.35;
                    
                    camera.position.set(center.x - maxDim * 0.1, center.y + maxDim * 0.2, center.z + cameraZ); 
                    controls.update();
                }
            }
