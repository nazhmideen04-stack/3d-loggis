...
# =========================================================================
# ЕДИНЫЙ И МОЩНЫЙ МЕТОД ЗАГРУЗКИ ЧЕРЕЗ CSV (ГАРАНТИЯ ВСЕХ ДАННЫХ)
# =========================================================================
@st.cache_data(ttl=900)
def fetch_csv_database():
    """Скачивает CSV для каждой категории через ALL и сохраняет всю историю в память"""
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Открываем Types и выбираем ALL для полной выгрузки
            ...
            for cat_key, cat_cfg in CATEGORIES.items():
                ...
                # Выбор категории с перебором вариантов названий
                success = False
                ...
                try:
                    ...
                    with page.expect_download(timeout=45000) as d_info:
                        try:
...
# Загружаем всю базу через CSV
with st.spinner("LoggIS veritabanı indiriliyor ve sensörler senkronize ediliyor (15-30 sn)..."):
    all_dates, full_db = fetch_csv_database()

# Строим древовидный каталог дат
date_tree = {}
if all_dates:
...
# ---------------------------------------------------------
# ЛОГИКА ВЫБОРА ВРЕМЕНИ И КАТЕГОРИЙ
# ---------------------------------------------------------
...
if not all_dates:
    ...
    if data_mode == "🔴 Canlı (En Güncel) Veriler":
        # Берем самый верхний элемент отсортированного списка — это абсолютный свежак
