    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                errors_log.append(f"Период ({mode_type}) не выбран: {e}")

            # Раскрываем меню Types
            try:
                page.get_by_text("Types").first.click()
                page.wait_for_timeout(1000)
            except Exception as e:
                errors_log.append(f"Кнопка 'Types' не нажата: {e}")
                ...
                try:
                    ...
                    try:
                        with page.expect_download(timeout=25000) as download_info:
                            try:
                                with page.expect_popup(timeout=6000) as p_info:
                        ...
                        errors_log.append(f"Категория [{opt_name}]: Не удалось скачать CSV (таймаут кнопки или выгрузки).")
                        continue

                    if not csv_path or not os.path.exists(csv_path) or os.path.getsize(csv_path) == 0:
                        errors_log.append(f"Категория [{opt_name}]: Сервер сформировал пустой файл (0 байт).")
                        continue

                    # Разбор CSV через pandas
                    try:
                        df_raw = pd.read_csv(csv_path, sep=';', skiprows=0, encoding='utf-8-sig', dtype=str)
                        if df_raw.shape[1] <= 1:
                            df_raw = pd.read_csv(csv_path, sep=',', skiprows=0, encoding='utf-8-sig', dtype=str)
                    except Exception as parse_err:
                        errors_log.append(f"Категория [{opt_name}]: Ошибка чтения CSV структуры: {parse_err}")
                        continue

                    if df_raw.empty or len(df_raw.columns) < 2:
                        errors_log.append(f"Категория [{opt_name}]: Файл пуст или в нем отсутствуют столбцы измерений.")
                        continue

                    date_col_name = df_raw.columns[0]
                    sensor_columns = df_raw.columns[1:]

                    # Фильтрация колонок строго под категорию
                    valid_cols = []
                    for c in sensor_columns:
                        c_upper = c.upper()
                        if target_tag == '-CS' and '-CS' in c_upper:
                            valid_cols.append(c)
                        elif target_tag == '-S' and '-S' in c_upper and '-CS' not in c_upper:
                            valid_cols.append(c)
                        elif target_tag == '-TP' and ('-TP' in c_upper or 'TEMP' in c_upper):
                            valid_cols.append(c)

                    if not valid_cols:
                        errors_log.append(f"Категория [{opt_name}]: В файле нет датчиков с индексом '{target_tag}'. Колонки в CSV: {list(sensor_columns[:6])}")
                        continue
                    ...
                    for _, row in df_raw.iterrows():
                        ...
                        date_key = normalize_date_key(str(raw_date))
                        dates_set.add(date_key)
                        
                        val_map = {}
                        for col in valid_cols:
                            m = re.search(r"(T[AB]-[A-Za-z0-9\-]+)", col)
                            sensor_id = m.group(1) if m else col.split()[0].strip()
                            
                            val = clean_num(row[col])
                            if not np.isnan(val):
                                val_map[sensor_id] = val
                    ...
                    if valid_points_count == 0:
                        errors_log.append(f"Категория [{opt_name}]: Датчики присутствуют в таблице, но все значения пустые (NaN). На сервере LoggIS нет замеров за этот период.")

                except Exception as cat_err:
                    errors_log.append(f"Ошибка при обработке [{opt_name}]: {cat_err}")

        except Exception as e:
            errors_log.append(f"Сбой подключения Playwright к LoggIS: {e}")
        finally:
            browser.close()

    sorted_dates = sorted(list(dates_set), reverse=True)
    return sorted_dates, historical_db, errors_log
...
with col_nav:
    ...
    if data_mode == "Arşiv Veriler":
        ...
        with st.spinner("Arşiv verileri yükleniyor..."):
            all_dates, full_db, errors_list = fetch_csv_database(mode_type="ALL")
